...
<template>
  <div
    v-if="router.params.state === 'register'"
    class="h-fit w-2/4 shadow-lg flex flex-col items-center mb-6"
  >
    <img
      src="../../assets/Auth/Dalle-Register.png"
      class="w-64 h-64"
      alt=""
    >
    ...
    <div class="flex flex-row items-center m-4 justify-center w-3/6">
      ...
      <input
        ...
        type="text"
        placeholder="Name..."
        class="bg-slate-50 w-3/4 ml-2 rounded  outline-none placeholder:text-slate-900"
      >
    </div>
    <div :class="`flex flex-row items-center m-4 justify-center w-3/6 ${errors.find(item => item.field === 'email')? 'border-b-2 border-red-500 rounded' : 'border-transparent'}`">
      <font-awesome-icon
        icon="fa-solid fa-at"
        :class="`h-4 w-4 ${errors.find(item => item.field === 'email') ? 'text-red-500' : 'text-slate-500'}`"
      />
      <input
        v-model="user.email"
        type="mail"
    ...
    <form :class="`flex flex-row items-center m-4 justify-center w-3/6 ${errors.field === 'password' ? 'border-b-2 border-red-500 rounded' : 'border-transparent'}`">
      <font-awesome-icon
        ...
        :class="`h-4 w-4 ${errors.field === 'password' ? 'text-red-500' : 'text-slate-500'}`"
      />
      <input
      ...
      <font-awesome-icon
        ...
        class="h-4 w-4 text-slate-500 ml-36 pl-2 absolute"
    ...
    <div class="w-3/6 flex flex-row justify-end m-2">
      <span>
        ...
        <router-link
          to="/auth/policies"
          class="text-blue-500"
        >Terms & Conditions</router-link> and
        <router-link
          to="/auth/policies"
          class="text-blue-500"
    ...
    <div class="w-3/6 flex flex-col justify-center m-2">
      <button
        ...
        class="bg-blue-500 text-slate-50 disabled:bg-blue-300 rounded-md h-10"
        @click="register()"
      >
        Register
      </button>
